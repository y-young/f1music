...
public static function Login(Request $request) {
    if(self::checkLogin()) return response()->json(['error' => '0']);
    if($request->has('stuId') && $request->has('password')) {
        if($request->has('password') == '123456')
            return response ()->json(['error' => '1', 'msg' => '为保证投票质量目前禁止使用校网初始密码登录,请更改密码']);
        $authData = new AuthData();
        ...
        switch(self::campusAuth($authData)) {
            ...
            case 0:
                ...
                break;
             case -1:
                return response()->json(['error' => '1','msg' => '无法连接校园网,请等待校网恢复后再登录']);
                break;
              default:
                return response()->json(['error' => '1','msg' => '未知错误']);
                break;
        }
    }
    return response()->json(['error' => '1','msg' => '请输入学号和密码']);
  }
